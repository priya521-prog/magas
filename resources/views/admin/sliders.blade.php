                        ...
                        <table class="table table-responsive table-bordered slideShow">
                            @foreach($sliders as $slider)
                                <tr>
                                    ...
                                    <td>
                                        <input type="url" placeholder="url" name="slider_url" data-id="{{ $slider->id }}" class="form-control" value="{{ $slider->url }}" />
                                        <br />
                                        <!--<a href="javascript:;" class="btn btn-danger btn-lg imgDeleteBtn" data-id="{{ $slider->id }}"><i class="fa fa-trash"></i> </a>-->
                                    </td>
                                    <td>
                                         
                                         <a class="btn theme-btn" style="background-color:#191a50;" href="{{  route('editslider', [$slider->id]) }}">Edit </a>
                                    </td>
                                </tr>
                            @endforeach
                        </table>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('keyup change', 'input[name="slider_url"]', function () {
              ...
              //  alert(slider_id);
                $.ajax({
                    url: '{{ route('update_slider_url') }}',
                    type: "POST",
                    data: {media_id: slider_id, url: url, _token: '{{ csrf_token() }}'}
                });
            });
